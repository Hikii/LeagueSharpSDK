...
namespace hVayne.Extensions
{
    class Condemn : Spells
    {
        ...
        public static void AsunaCondemn(Obj_AI_Hero target, int push)
        {
            ...
            for (var i = 1; i < pushDist; i += (int)target.BoundingRadius)
            {
                ...
                if (loc3.IsWall() || collFlags.HasFlag(CollisionFlags.Wall) || collFlags.HasFlag(CollisionFlags.Building))
                {
                    E.Cast(target);
                }   
            }
